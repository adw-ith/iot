...
catch(error)
{
  ...
  if(newName)
  {
    try {
            ...
            if(usersRes3[i].borrowed_components)
            {
               for(var j=0;j<usersRes3[i].borrowed_components.length;j++)
               {
                if(usersRes3[i].borrowed_components[j].name ===name)
                {
                    return new Response(
                        JSON.stringify({
                          message: "component exists in the users. Remove it from borrowed_components before proceeding",
                        }),
                        {
                          status: 306,
                          headers: {
            ...
            if(usersRes3[i].requested_components)
            {
               for(var j=0;j<usersRes3[i].requested_components.length;j++)
               {
                if(usersRes3[i].requested_components[j].name ===name)
                {
                    return new Response(
                        JSON.stringify({
                          message: "component exists in the users. Remove it from requested_components before proceeding",
                        }),
                        {
                          status: 305,
                          headers: {
        ...
        const existing = await db.collection("components").findOne({ name:newName });
        if (existing) {
          return new Response(
            JSON.stringify({
